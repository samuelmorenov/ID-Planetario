...
Mundo::Mundo(char **nombre) {

InitParser(nombre);//NUEVO
...
void Mundo::InitParser(char **nombre){//NUEVO

    xmlparser * parser;
    xmlnode * gameXML;
    string line,xml;
    ifstream textstream;
    textstream.open(nombre[2]);
    if (!textstream.is_open()){
            cout << endl << ">> Error al abrir el archivo " << nombre[2] << "<<"<< endl << endl;
            exit(1);
        }
...
void Mundo::DrawGLScene() {
    ...
    for (int i =0; i<numModelos; i++){
        ...
        if(modelos[i].esLuna){
            ...
            glTranslatef(modelos[f].radioGiro,0,0);

            dibujarCirculo(modelos[i].radioGiro);
            glRotatef(angulo_float, 0.0f, 0.0f, 10.0f);
            glTranslatef(modelos[i].radioGiro,0,0);
